...
const CommunityPage = () => {
  const { user } = useAuth();
  const [messages, setMessages] = useState(MOCK_DATA.communityChat);
                ...
                {messages.map((message, index) => (
                  <motion.div
                    ...
                    className={`flex items-start space-x-3 ${
                      user && message.user === user.name ? 'flex-row-reverse space-x-reverse' : ''
                    }`}
                  >
                    {/* Avatar */}
                    <div className="flex-shrink-0">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white ${
                        user && message.user === user.name 
                    ...
                    <div className={`flex-1 max-w-xs sm:max-w-md ${
                      user && message.user === user.name ? 'text-right' : ''
                    }`}>
                      <div className={`flex items-baseline space-x-2 mb-1 ${
                        user && message.user === user.name ? 'justify-end' : ''
                      }`}>
